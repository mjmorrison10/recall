...
(function () {
  ...
  // caller's catch/finally never run and the button freezes forever.
  //
  // The deadline MUST span the body read, not just the headers: OpenRouter's
  // non-streaming endpoint returns 200 headers immediately and then holds the
  // connection open for the whole generation, so a headers-only timeout never
  // fires — the await on res.json() just sits there with no error at all.
  ...
  var OP_BUDGET_MS = 240000;            // whole operation, retries included

  // Resolves to { res, bodyText } — response plus fully-read body, both under
  // one deadline. Callers parse bodyText instead of awaiting res.json(), which
  // would be a second, unbounded wait.
  ...
  // optional/best-effort. Stops once OP_BUDGET_MS would be exceeded, so the
  // worst case is bounded instead of attempts × timeout + backoff.
  async function fetchWithRetry(makeRequest, onRetry) {
    ...
    for (var attempt = 1; ; attempt++) {
      ...
      if (Date.now() - started + wait > OP_BUDGET_MS) return out;
